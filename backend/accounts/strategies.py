...
def kalman_long(df: pd.DataFrame, *, fee_side: float = 0.001, sl_pct: float = 0.05, tp_pct: float = 0.10,
    ...
    equity = np.ones(len(sig), dtype="float64")
    trades: list[dict] = []
    in_pos = False; ent_idx = None; ent_price = np.nan; bars = 0
    for i in range(1, len(sig)):
        if in_pos:
            hit, px, why = _exit_long_by_stops(o.iat[i], h.iat[i], l.iat[i], ent_price, sl_pct, tp_pct)
            if hit:
        ...
        if in_pos and sell.iat[i]:
            _append_trade(trades, "long", sig.index[ent_idx], ent_price, sig.index[i], c.iat[i], bars+1, fee_side); trades[-1]["reason"] = "signal"
            in_pos = False; ent_idx = None; ent_price = np.nan; bars = 0
            equity[i] = equity[i-1] * trades[-1]["net_factor"]; continue
        if (not in_pos) and buy.iat[i]:
            in_pos = True; ent_idx = i; ent_price = c.iat[i]; bars = 0; equity[i] = equity[i-1]; continue
        if in_pos: bars += 1
        equity[i] = equity[i-1]
    ...
    for i in range(1, len(sig)):
        if in_pos:
            ...
            if hit:
                ...
                in_pos = False; ent_idx = None; ent_price = np.nan; bars = 0
                equity[i] = equity[i-1] * trades[-1]["net_factor"]; continue
        if in_pos and buy.iat[i]:
            _append_trade(trades, "short", sig.index[ent_idx], ent_price, sig.index[i], c.iat[i], bars+1, fee_side); trades[-1]["reason"] = "signal"
            in_pos = False; ent_idx = None; ent_price = np.nan; bars = 0
            equity[i] = equity[i-1] * trades[-1]["net_factor"]; continue
        if (not in_pos) and sell.iat[i]:
            in_pos = True; ent_idx = i; ent_price = c.iat[i]; bars = 0; equity[i] = equity[i-1]; continue
        if in_pos: bars += 1
        equity[i] = equity[i-1]
    trades_df = pd.DataFrame(trades)
    ...
    for i in range(1, len(sig)):
        ...
        if buy.iat[i]:
            if pos == -1:
                ...
                pos = 1; ent_idx = i; ent_price = c.iat[i]; bars = 0
            elif pos == 0:
                pos = 1; ent_idx = i; ent_price = c.iat[i]; bars = 0; equity[i] = equity[i-1]
            else:
                equity[i] = equity[i-1]
        elif sell.iat[i]:
            if pos == 1:
                _append_trade(trades, "long", sig.index[ent_idx], ent_price, sig.index[i], c.iat[i], bars+1, fee_side); trades[-1]["reason"] = "signal_flip"
                equity[i] = equity[i-1] * trades[-1]["net_factor"]
                pos = -1; ent_idx = i; ent_price = c.iat[i]; bars = 0
            elif pos == 0:
                pos = -1; ent_idx = i; ent_price = c.iat[i]; bars = 0; equity[i] = equity[i-1]
